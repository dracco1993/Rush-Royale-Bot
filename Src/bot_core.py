...
class Bot:
    ...
    def shell(self, cmd):
        p = Popen(
            [".scrcpy\\adb", "-s", self.device, "shell", cmd],
            stdout=DEVNULL,
            stderr=DEVNULL,
        )
        p.wait()
    ...
    def merge_special_unit(self, df_split, merge_series, special_type):
        ...
        special_df, normal_df = [
            df_split.get_group(unit.index[0]).sample()
            for unit in [special_unit, normal_unit]
        ]
    ...
    def battle_screen(self, start=False, pve=True, floor=5):
        ...
        if not df.empty:
            # list of buttons
            if (df == "fighting.png").any(axis=None) and not (
                df == "0cont_button.png"
            ).any(axis=None):
                return df, "fighting"
            ...
            if (df == "home_screen.png").any(axis=None) and (
                df == "battle_icon.png"
            ).any(axis=None):
                if pve and start:
                ...
                time.sleep(10)  # TODO: check that this is right
                return df, "home"
    ...
    def watch_ads(self):
        ...
        if (
            status == "menu"
            or status == "home"
            or (avail_buttons == "refresh_button.png").any(axis=None)
        ):
...
def get_grid():
    ...
    # x_cords
    x_cord = list(
        range(top_box[0], top_box[0] + (box_size[0] + gap) * width, box_size[0] + gap)
    )
    y_cord = list(
        range(top_box[1], top_box[1] + (box_size[1] + gap) * height, box_size[1] + gap)
    )
    boxes = []
