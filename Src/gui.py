...
class RR_bot:
    def __init__(self):
        ...
        start_button = Button(
            self.frames[2], text="Start Bot", command=self.start_command
        )
        stop_button = Button(
    ...
    def update_config(self):
        ...
        self.config["bot"]["mana_level"] = np.array2string(card_level, separator=",")[
            1:-1
        ]
    ...
    def update_units(self):
        ...
        if not bot_handler.select_units(
            ...
            self.logger.info(
                f"Invalid units in config file! Valid units: {valid_units}"
            )
    ...
    def start_bot(self):
        ...
        thread_bot = threading.Thread(
            target=bot_handler.bot_loop, args=([bot, infos_ready])
        )
        thread_bot.start()
    ...
    def leave_game(self):
        ...
        if hasattr(self, "bot_instance"):
            thread_bot = threading.Thread(
                target=self.bot_instance.restart_RR, args=([True])
            )
            thread_bot.start()
        else:
...
def create_options(frame1, config):
    ...
    mana_buttons = [
        Checkbutton(
            frame1, text=f"Card {i+1}", variable=mana_vars[i], justify=LEFT
        ).grid(row=2, column=i + 1)
        for i in range(5)
    ]
    # Dungeon Floor
    floor_label = Label(frame1, text="Dungeon Floor", justify=LEFT).grid(
