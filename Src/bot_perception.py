...
def grid_status(bot_path, names, prev_grid=None):
    units_path = f"{bot_path}/units"

    ref_units = os.listdir(units_path)
    ref_colors = [get_color(f"{units_path}/{unit}")[0] for unit in ref_units]
    grid_stats = []
    ...
    if not prev_grid is None:
        # Check Consistency
        consistency = (
            grid_df[["grid_pos", "unit", "rank"]]
            == prev_grid[["grid_pos", "unit", "rank"]]
        )
...
def add_grid_to_dataset():
    for slot in os.listdir("OCR_inputs"):
        ...
        cv2.imwrite(
            f"machine_learning/inputs/{rank_guess}_input_{example_count}.png", edges
        )
        cv2.imwrite(
            f"machine_learning/raw_input/{rank_guess}_raw_{example_count}.png", img
        )
