...
def bot_loop(bot, info_event):
    ...
    max_loops = int(
        config.get("max_loops", 100)
    )  # this will increase time waiting when logging in from mobile
    ...
    while not bot.bot_stop:
        ...
        if output[1] == "fighting":
            ...
            if combat > max_loops:
                ...
                combat = 0
                continue
            elif require_shaman and not (output[0] == "shaman_opponent.png").any(
                axis=None
            ):
                ...
                if any(
                    [
                        (
                            bot.battle_screen(start=False)[0] == "shaman_opponent.png"
                        ).any(axis=None)
                        for i in range(1)
                    ]
                ):
            ...
            output = bot.battle_screen(
                start=True, pve=user_pve, floor=user_floor
            )  # (only 1,2,4,5,7,8,10 possible)
            wait += 1
            if wait > 40:
